...
with sync_playwright() as p:
    ...
    apage.wait_for_timeout(100)
    prog = apage.evaluate("() => ({ posted: window.__posted, bar: document.getElementById('LogoArtPersonsCreateBar').style.display, fill: document.getElementById('LogoArtPersonsCreateFill').style.width, text: document.getElementById('LogoArtPersonsCreateText').textContent, cancel: document.getElementById('LogoArtPersonsCreateCancelBtn').style.display, btn: document.getElementById('LogoArtPersonsCreateBtn').disabled })")
    check('S134b: OK starts the job and the status is drawn as a progress bar with counts + current name + Cancel', prog['posted'] == ['/LogoArt/create-logos'] and prog['bar'] == 'block' and prog['fill'] == '25%' and prog['text'] == '250 / 1 000 · 200 written · 50 skipped · Keanu Reeves' and prog['cancel'] == 'inline-block' and prog['btn'] is True, str(prog))
    apage.evaluate("""() => { ApiClient.ajax = function (o) { window.__posted.push(o.url); return Promise.resolve({ Running: false, Persons: 1000, Done: 1000, Written: 900, Skipped: 100, Failed: 0, Message: 'Done: 900 written, 100 skipped, 0 failed of 1000 persons.' }); }; document.getElementById('LogoArtPersonsCreateCancelBtn').click(); }""")
    apage.wait_for_timeout(100)
    done = apage.evaluate("() => ({ posted: window.__posted, bar: document.getElementById('LogoArtPersonsCreateBar').style.display, text: document.getElementById('LogoArtPersonsCreateText').textContent, cancel: document.getElementById('LogoArtPersonsCreateCancelBtn').style.display, btn: document.getElementById('LogoArtPersonsCreateBtn').disabled })")
    check('S134b: Cancel posts create-logos/cancel; a finished job leaves the summary, hides bar + Cancel, frees the button', done['posted'][-1] == '/LogoArt/create-logos/cancel' and done['bar'] == 'none' and done['text'].startswith('Done: 900 written') and done['cancel'] == 'none' and done['btn'] is False, str(done))
