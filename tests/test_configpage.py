...
with sync_playwright() as p:
    ...
    # ─── Session 134: LogoArt tab (concept docs/artworkplus-logoart-concept.md, Fibel Part C row; 134b: geometry per stage, Take over, Persons Off) ───
    apage.evaluate("""() => { document.getElementById('epRestoreAllBtn').click(); }""")
    apage.wait_for_timeout(150)
    ...
    check('S134: General switch row exists, no in-tab Enable, no format list',
          apage.evaluate("() => !!document.getElementById('LogoArtEnabledRow') && !document.getElementById('LogoArtTabEnabled') && !document.getElementById('LaFormat_png_chk') && document.querySelectorAll('.epTabPage[data-tabpage=logoart] .epFormatGrid').length === 0"))
    ...
    present = apage.evaluate("(types) => types.map(function (k) { var p = 'LogoArt' + k; return ['Source', 'Fallback', 'SecondFallback', 'LogoSizePercent', 'LogoOffsetVw', 'LogoVerticalOffsetVh', 'ClearartSizePercent', 'ClearartOffsetVw', 'ClearartVerticalOffsetVh', 'DependentFields', 'OwnFields', 'LogoFields', 'ClearartFields'].every(function (s) { return !!document.getElementById(p + s); }); })", types)
    check('S134b: ten type blocks with chain + Logo and Clearart geometry', all(present), str(present))
    ...
    cab = apage.evaluate("(types) => types.map(function (k) { var q = 'LogoArt' + k + 'Characterart'; return !!document.getElementById(q + 'Fields') && !!document.getElementById(q + 'ScaleMode') && !!document.getElementById(q + 'HeightRow') && !!document.getElementById(q + 'WidthRow') && !!document.getElementById(q + 'AlignRow') && !!document.getElementById(q + 'OffsetRow') && !!document.getElementById('LogoArt' + k + 'RotationFields'); })", types)
    check('S134b: Characterart sizing block (scale by / height / width / align / offset) + rotation only where Characterart is possible', cab == ca, str(cab))
    takeover = apage.evaluate("(types) => types.map(function (k) { return !!document.getElementById('LogoArt' + k + 'Settings'); })", types)
    check('S134b: Settings switch only for Set / Season / Episode', takeover == [False, False, True, True, True, False, False, False, False, False], str(takeover))
    ...
    check('S134b: defaults (zero intervention, Characterart 20 vh / max 12 vw like the Characterart tab, Take over on)', defaults == ['VanillaLogo', 'None', 'None', 'Default', '100', '0', '0', '100', 'Height', '20', '12', 'TakeOverMovies', 'TakeOverSeries', 'TakeOverSeries'], str(defaults))
    ...
    check('S134b: Characterart fallback: Characterart + rotation rows active', agreyed('LogoArtMovieCharacterartFields') is False and agreyed('LogoArtMovieRotationFields') is False)
    check('S134b: scale by Height: height row active, width row grey', agreyed('LogoArtMovieCharacterartHeightRow') is False and agreyed('LogoArtMovieCharacterartWidthRow') is True)
    check('S134b: Align active with the default max width 12 (Characterart tab rule)', agreyed('LogoArtMovieCharacterartAlignRow') is False)
    apage.evaluate("() => { var e = document.getElementById('LogoArtMovieCharacterartMaxWidthVw'); e.value = '0'; e.dispatchEvent(new Event('input', { bubbles: true })); e.dispatchEvent(new Event('change', { bubbles: true })); }")
    check('S134b: Align grey once max width is 0', agreyed('LogoArtMovieCharacterartAlignRow') is True)
    apage.evaluate("() => { var e = document.getElementById('LogoArtMovieCharacterartMaxWidthVw'); e.value = '12'; e.dispatchEvent(new Event('input', { bubbles: true })); e.dispatchEvent(new Event('change', { bubbles: true })); }")
    sel('LogoArtMovieCharacterartScaleMode', 'Width')
    check('S134b: scale by Width swaps the rows', agreyed('LogoArtMovieCharacterartWidthRow') is False and agreyed('LogoArtMovieCharacterartHeightRow') is True)
    sel('LogoArtMovieCharacterartScaleMode', 'Height')
    sel('LogoArtMovieSecondFallback', 'VanillaLogo')
    check('S134b: all three kinds in the chain: every geometry block active', agreyed('LogoArtMovieLogoFields') is False and agreyed('LogoArtMovieClearartFields') is False and agreyed('LogoArtMovieCharacterartFields') is False)
    # chain rule (epSyncSetFallback pattern)
    sel('LogoArtMovieSource', 'VanillaLogo')
    val = apage.evaluate("() => [document.getElementById('LogoArtMovieFallback').value, document.getElementById('LogoArtMovieSecondFallback').value]")
    check('S134: a fallback equal to the new source falls back to None', val[1] == 'None' and val[0] == 'Characterart', str(val))
    ...
    check('S134: the fallback cannot repeat the source', dis == ['VanillaLogo'], str(dis))
    ...
    check('S134: Hide as source greys both fallback rows and every geometry block', agreyed('LogoArtMovieFallbackRow') is True and agreyed('LogoArtMovieSecondFallbackRow') is True and agreyed('LogoArtMovieLogoFields') is True and agreyed('LogoArtMovieCharacterartFields') is True)
    sel('LogoArtMovieSource', 'VanillaLogo')
    ...
    check('S134: Playback select drives the hidden checkbox (bool-select pair)', apage.evaluate("() => document.getElementById('LogoArtMovieSinglePass').checked") is True)
    ...
    sel('LogoArtMovieFallback', 'None')
    # Take over
    check('S134b: Set at Take over Movies: own rows grey', agreyed('LogoArtSetOwnFields') is True and agreyed('LogoArtSetSettingsRow') is False)
    sel('LogoArtSetSettings', 'Individual')
    check('S134b: Set Individual frees the own rows', agreyed('LogoArtSetOwnFields') is False and agreyed('LogoArtSetLogoFields') is False)
    sel('LogoArtSetSettings', 'TakeOverMovies')
    check('S134b: Episode at Take over Series: own rows grey, Source mode stays active', agreyed('LogoArtEpisodeOwnFields') is True and agreyed('LogoArtEpisodeSourceModeRow') is False)
    sel('LogoArtEpisodeSettings', 'Individual')
    check('S134b: Episode Individual frees the own rows', agreyed('LogoArtEpisodeOwnFields') is False)
    sel('LogoArtEpisodeSettings', 'TakeOverSeries')
    ...
    # Persons
    pers = apage.evaluate("() => ['LogoArtPersonsSource', 'LogoArtPersonsFallback', 'LogoArtPersonsBaseName', 'LogoArtPersonsFontPool', 'LogoArtPersonsFonts', 'LogoArtPersonsPreviewNames', 'LogoArtPersonsTextStroke', 'LogoArtPersonsOutline', 'LogoArtPersonsUppercase', 'LogoArtPersonsSizePercent', 'LogoArtPersonsOffsetVw', 'LogoArtPersonsVerticalOffsetVh', 'LogoArtPersonsCreateBtn', 'LogoArtPersonsCreateMode', 'LogoArtPersonsCreateResult', 'LogoArtPersonsCreateBar', 'LogoArtPersonsFontsCheckAllBtn', 'LogoArtPersonsFontsUncheckAllBtn', 'LogoArtPersonsFontsList', 'LogoArtPersonsFontsPreview', 'LogoArtPersonsFontsBtn', 'LogoArtPersonsFontsPanel', 'LogoArtPersonsActiveFields'].map(function (id) { return !!document.getElementById(id); })")
    check('S134: Persons block complete (chain, Base name, pool, dropdown, preview, stroke/outline/uppercase, creator + progress, geometry)', all(pers), str(pers))
    pd = apage.evaluate("() => [document.getElementById('LogoArtPersonsSource').value, Array.prototype.map.call(document.getElementById('LogoArtPersonsSource').options, function (o) { return o.value + '=' + o.text; }).join('|'), document.getElementById('LogoArtPersonsFallback').value, document.getElementById('LogoArtPersonsBaseName').value, document.getElementById('LogoArtPersonsFontPool').value, document.getElementById('LogoArtPersonsFonts').value, document.getElementById('LogoArtPersonsTextStroke').value, document.getElementById('LogoArtPersonsOutline').value, document.getElementById('LogoArtPersonsUppercase').checked]")
    check('S134b: Persons Source Off (default) / Folder logo / Text, defaults Off, Text, clearlogo, Signature, *, 0, 1, off', pd == ['Off', 'Off=Off (default)|FolderLogo=Folder logo|Text=Text', 'Text', 'clearlogo', 'Signature', '*', '0', '1', False], str(pd))
    check('S134b: Persons Off greys everything below Source', agreyed('LogoArtPersonsActiveFields') is True and agreyed('LogoArtPersonsSourceRow') is False)
    ...
    # the font dropdown with a stub manifest (the stub ApiClient has no ajax - the page uses the fallback list)
    apage.evaluate("""() => { window.epLogoArtFontsFallback = [
        { group: 'signature', family: 'Alpha Sig', file: 'Signature/AlphaSig.otf', licence: 'x', previewEm: 8, previewUpperEm: 12 },
        { group: 'signature', family: 'Beta Sig', file: 'Signature/BetaSig.otf', licence: 'x', previewEm: 16, previewUpperEm: 30 },
        { group: 'title', family: 'Gamma Title', file: 'Title/GammaTitle.otf', licence: 'x', previewEm: 10, previewUpperEm: 12 } ]; epLogoArtLoadFonts(); }""")
    apage.wait_for_timeout(50)
    ...
    apage.evaluate("() => { var h = document.querySelector('.epCollapseHeader[data-collapse=logoartPersons]'); if (!h.classList.contains('epOpen')) { h.click(); } }")
    ...
    apage.wait_for_timeout(50)
    nf = apage.evaluate("() => { var r = document.getElementById('LogoArtPersonsCreateResult'); return { msgs: window.__confirmMsgs.length, posted: window.__posted.length, text: document.getElementById('LogoArtPersonsCreateText').textContent, color: r.style.color, shown: r.style.display, h: r.getBoundingClientRect().height, bar: document.getElementById('LogoArtPersonsCreateBar').style.display, noCancel: !document.getElementById('LogoArtPersonsCreateCancelBtn') }; }")
    check('S134e: no font selected -> only the red one-line message (API-key pattern), no bar, no warning, no request', nf['msgs'] == 0 and nf['posted'] == 0 and nf['text'] == 'No font(s) selected - select at least one.' and nf['color'] == 'rgb(255, 138, 138)' and nf['shown'] == 'block' and 14 <= nf['h'] <= 20 and nf['bar'] == 'none' and nf['noCancel'], str(nf))
    apage.evaluate("() => document.getElementById('LogoArtPersonsFontsCheckAllBtn').click()")
    ...
    check('S134b: Create logos shows the warning (text C) and sends nothing on Cancel', len(cw['msgs']) == 1 and cw['msgs'][0] == 'This will create thousands of .png files in ProgramData\\Jellyfin\\Server\\metadata\\People. No undo. Continue?' and cw['posted'] == [], str(cw))
    apage.evaluate("""() => { window.confirm = function () { return true; }; document.getElementById('LogoArtPersonsCreateBtn').click(); }""")
    apage.wait_for_timeout(100)
    prog = apage.evaluate("() => { var r = document.getElementById('LogoArtPersonsCreateResult'); return { posted: window.__posted, bar: document.getElementById('LogoArtPersonsCreateBar').style.display, fill: document.getElementById('LogoArtPersonsCreateFill').style.width, text: document.getElementById('LogoArtPersonsCreateText').textContent, color: r.style.color, h: r.getBoundingClientRect().height, btn: document.getElementById('LogoArtPersonsCreateBtn').disabled }; }")
    check('S134b: OK starts the job and the status is one line: progress bar + counts + current name (grey while running)', prog['posted'] == ['/LogoArt/create-logos'] and prog['bar'] == 'block' and prog['fill'] == '25%' and prog['text'] == '250 / 1 000 · 200 written · 50 skipped · Keanu Reeves' and prog['color'] == 'rgb(153, 153, 153)' and 14 <= prog['h'] <= 20 and prog['btn'] is True, str(prog))
    # Session 138 (audit S3-05): the running job's 1 s poll stops on viewhide and resumes on viewshow
    apage.evaluate("""() => { window.__posted = []; }""")
    apage.evaluate("""() => { document.getElementById('ArtworkPlusConfigPage').dispatchEvent(new CustomEvent('viewhide')); }""")
    apage.wait_for_timeout(2300)
    hidden_polls = apage.evaluate("() => window.__posted.filter(function (u) { return u.indexOf('create-logos/status') !== -1; }).length")
    apage.evaluate("""() => { document.getElementById('ArtworkPlusConfigPage').dispatchEvent(new CustomEvent('viewshow')); }""")
    apage.wait_for_timeout(100)
    shown_polls = apage.evaluate("() => window.__posted.filter(function (u) { return u.indexOf('create-logos/status') !== -1; }).length")
    check('S138: no status poll while the view is hidden, one pickup request on viewshow', hidden_polls == 0 and shown_polls == 1, 'hidden=%s shown=%s' % (hidden_polls, shown_polls))
    apage.evaluate("""() => { epLogoArtShowJob({ Running: false, Persons: 1000, Done: 1000, Written: 900, Skipped: 100, Failed: 0, Message: 'Done: 900 written, 100 skipped, 0 failed of 1000 persons.' }); }""")
    done = apage.evaluate("() => { var r = document.getElementById('LogoArtPersonsCreateResult'); return { bar: document.getElementById('LogoArtPersonsCreateBar').style.display, text: document.getElementById('LogoArtPersonsCreateText').textContent, color: r.style.color, btn: document.getElementById('LogoArtPersonsCreateBtn').disabled }; }")
    check('S134b: a finished job leaves the green summary, hides the bar, frees the button', done['bar'] == 'none' and done['text'].startswith('Done: 900 written') and done['color'] == 'rgb(136, 255, 136)' and done['btn'] is False, str(done))
