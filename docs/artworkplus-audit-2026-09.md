...
| ID | file:line | class | sev | finding | evidence | know/believe | recommendation | a fix could break |
|---|---|---|---|---|---|---|---|---|
| S4-01 | `ArtworkPlus.csproj` (SixLabors.ImageSharp 3.1.7) | dependency advisory | risk - FIXED (Session 138, deploy #76) | NU1902: 3.1.7 carries a known moderate advisory (GHSA-rxmq-m78w-7wmc). It matters here more than in most plugins: ImageSharp decodes images DOWNLOADED from the web (People Backdrops aspect check + text detector, `PeopleBackdropsController.cs:1379-1394`), i.e. untrusted input, reachable through the anonymous endpoint (S1-01). The 3.1.x line stays under the free split licence without a key (the pin at 3.1.7 was for the 4.0 licence-key rule, not for 3.1.7 itself). **Fix:** SixLabors.ImageSharp 3.1.7 -> 3.1.12 (latest 3.1.x, no licence key); NU1902 gone from the build; People population flow exercised live after the deploy. | build warning; csproj comment; download path | know | bump to the latest 3.1.x patch (no licence key, same API), rebuild, run the People smoke | nothing expected - patch line; verify `PassesAspectRatioCheck` and the text detector on the live People fixture |
