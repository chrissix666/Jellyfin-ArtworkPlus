...
| S1-10 | `docs/artworkplus-feature-map.md` Backdrops row | doc | doc | The endpoint column lists 4 of the 15 `Backdrops` routes (`genre-pool`, `studio-pool`, `studio-settings`, `studio-image`, `tag-pool`, `library-pool`, `favorites-pool`, `favorites-people-pool`, `script.js` are only in the Fibel text). | routes vs map | know | complete the row (area 5) | nothing |
...
· doc 1. Top by risk: S1-03 (log volume, measurable cost every day), S1-01
