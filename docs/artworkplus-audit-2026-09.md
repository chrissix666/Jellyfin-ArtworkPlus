...
|---|---|---|---|---|---|---|---|---|
| S1-01 | `Controllers/PeopleBackdropsController.cs:285-330, 1340-1400` | amplification | risk / design - cap FIXED (Session 138, deploy #81), auth = design | The anonymous `GET /PeopleBackdrops/{personId}` triggers, for a person without cache file, a Wallpapers.com search plus a full-size `GetByteArrayAsync` download, decode and text detection per candidate image and a cache-file write - no response size cap, no per-client limit. Bounded by: valid person GUIDs needed, `PopulateLocks` dedupes concurrent calls per person, the 7-day empty-cache retry, `MaxImages` <= 10. **Design note:** anonymous by design (the person page fetches the stream like an image); the missing download cap is the only part that is not a decision. Changing the auth touches the client - park until area 2. **Fix (part 1):** `HttpClient.MaxResponseContentBufferSize = 25 MB` for the candidate downloads - an oversized answer throws into the existing download-failed branch and is skipped. **Part 2 (auth) stays design** while the server is LAN-only: the client fetches the stream without a token (S2-08); the day the server is exposed, `[Authorize]` + the token header in `Backdrops-v1.js:488` is the first fix. | code read; probe with a random GUID -> 200 "not found" stream, no fetch | know (static + probe) | leave as is on a LAN server; if the server is exposed: `[Authorize]` on the endpoint (the page is logged in anyway) and a `MaxResponseContentBufferSize` on the HttpClient | client fetches the stream with the page's ApiClient? - verify in area 2 before adding auth |
| S1-02 | `Controllers/LogoArtController.cs:758-796` | state | smell (pattern) | If anything throws OUTSIDE the per-person try (the `LogoTextRenderer` constructor, the Fonts folder unreadable), the `Task.Run` faults, `_job.Running` stays true, `CreateLogos` answers "running" forever and `cancel` cannot reset it - only a server restart. **/fix dossier 2026-09-21 (Session 138): downgraded.** Every statement outside the per-person try was checked for throwability: `new LogoTextRenderer` (assignment only), `token.IsCancellationRequested`, `job.Current = person.Name`, `foreach` over a materialised `List<Person>` - none can throw; every real fault (path, file, font, SkiaSharp, write) is already inside the try and ends as `Failed++`. No red proof possible without artificial fault injection (rulebook point 4) -> no fix, pattern noted (a try/finally would be defensive only). | code read: the only catch is inside the loop | believe (not reproduced - would need a fault injection) | wrap the whole task body in try/finally that sets `Running=false` + `Message` | nothing |
| S1-03 | `AnimatedPosterController.cs:535`, `CustomPosterController.cs:~575`, `CharacterartController.cs:266,272`, `BackdropsController.cs:316`, `FileTransformationRegistrar.cs:489-642` | log volume / perf | risk / design - FIXED (Session 138, deploy #76) | Information-level lines per item and per image: on 2026-09-21 the plugin wrote 10 425 of the server's 13 016 log lines (80 %): `FindAnimatedFile` 3 511, `FindCustomPosterFile` 1 970 (one line per item per batch on a cache miss), `TransformIndexHtml` 823 (5-8 lines per index.html load), Characterart 786 + 475 + 384 + 183 (per page and per image incl. 304s), `GetSettings` 354. Lesson B25 measured ~1 ms per line on the synchronous console sink - a 500-card library batch with cold cache = ~1 s of logging. Also a `SafeGetLastWriteTimeUtc` stat per item per batch (cache-key design, external drives). **Design note:** the Information level was chosen in the sandbox to diagnose without switching Debug on; what was never decided is the per-item volume - the user re-decides the level, the audit only reports the cost. **Fix (user's decision: the lever, not removal):** 35 per-item / per-image / per-tag lines Information -> Debug (AnimatedPoster 3, CustomPoster 3, Characterart 17, RedCarpet 2, transform 10); every request-level summary stays; `logging.json` `"Jellyfin.Plugin.ArtworkPlus": "Debug"` brings all of them back without a rebuild. Proof: log lines per page kind before vs after (below). | log counted by message kind; code read | know | move the per-item / per-image lines to Debug (the batch START/END lines already carry counts), keep one Information line per request; consider `LogDebug` for the transform's per-tag lines | nothing functional; tests grep no log text |
| S1-04 | `BackdropsController.cs:402-410, 1303-1311, 1456-1487`, `PeopleBackdropsController.cs:761-855` | error handling | smell - FIXED (Session 138, deploy #70) | The four local-file endpoints without try/catch (`episode-image`, `custom-image`, `studio-image`, `folder-image`) answer 500 instead of 404 when the file disappears between the directory scan and `PhysicalFile` (every other file endpoint catches and logs). **Fix:** try/catch around the four actions plus `child-image` (a fifth unguarded action the new diagnostic found); `ServeLocalImage` and `child-image` open the stream inside the action (a `PhysicalFile` result opens outside the try). Guard: `tests/diagnostic_file_endpoints_guarded.py` in run_checks (21/21). | code read | know (proof before: locked person backdrop -> 500 + ExceptionMiddleware log line; after: 404 + warning) | same try/catch pattern as `GetPosterImage` | nothing |
| S1-04b | `CaseModController.cs:465`, `LogoArtController.cs:699, 831` (`GetFont`, `ServeFile`) | deferred open | smell - FIXED (Session 138, deploy #71) | Found by the S1-04 fix: `PhysicalFile(...)` opens the file when the RESULT executes, outside the action's try/catch - a locked font answers 500 (probe: `/LogoArt/font/Signature/Activity.otf` locked -> 500, unlocked -> 200) although the action is wrapped. Three sites left (CaseMod texture, LogoArt font, LogoArt ServeFile for characterart / person images). **Fix:** the three sites open the stream in the action (`File(File.OpenRead(...))`); the existing catches stay (ServeFile callers -> 404 + warning, font / texture -> their own 500 log line instead of the ExceptionMiddleware trace). Guard diagnostic: 0 deferred-open sites. | probe + code | know | open the stream in the action (`File(System.IO.File.OpenRead(path), type)`) as done for `ServeLocalImage` / `child-image` | nothing (same headers, same bytes) |
...
| S1-07 | `LogoArtController.cs:538-549, 773`, naming-mode folder names, `RedCarpetController.cs:261`, `BackdropsController.cs:355` | input hygiene | smell - FIXED (Session 138, deploy #80) | Admin-typed config strings go into `Path.Combine` unsanitised (`LogoArtPersonsBaseName`, the "Folder" naming-mode folder names, `RedCarpetFolderName`, `BackdropsEpisodeBaseName`): `..\..\x` escapes the item folder; `create-logos` would then WRITE `x.png` there. Admin-only setting - admins own the server anyway. **Fix:** `Plugin.UpdateConfiguration` override + `Helpers/ConfigNames.Sanitize`: on every save (page and API) the 25 base / type / folder name properties are reduced to one path segment (`..`, directory parts dropped) with a warning line; the 35 read sites untouched. Proof: `..\..\evil` and `sub/dir` were stored verbatim before, reduced to `evil` / `dir` after (config restored byte-identical). | code read | know | reject path separators / `..` in these fields on save (page) or `Path.GetFileName` on the server | nothing |
| S1-08 | `FileTransformationRegistrar.cs:504-506` | noise | smell - FIXED (Session 138, deploy #72) | The transform is registered for the pattern `index.html` and is also called with other HTML (three "no </body>" warnings on 2026-09-20/21 with content lengths 1 134 / 5 062 / 11 361 against the normal 11 108) - harmless, one warning each. **Fix:** the line is Debug with the byte length; every other line of the transform untouched. Proof: the three warnings in the 2026-09-20/21 logs (red); after: normal index loads still insert (smoke + page reload). | log counted (178 normal calls vs 3 odd ones) | know | log at Debug when the content is not the real index (no `</body>`) | nothing |
| S1-13 | `FileTransformation/FileTransformationRegistrar.cs:494-496` | error path | bug (latent) - FIXED (Session 138, deploy #81) | Found while fixing S1-08: if reading `payload.contents` throws, the transform returns `string.Empty` - depending on how the File Transformation plugin treats an empty return, the SPA index could be served blank. Never happened (the read never failed: 0 such log lines); needs a look at the File Transformation plugin's handling of an empty callback result before deciding whether to return the original or throw. **Verified in the File Transformation source (`Helpers/TransformationHelper.cs`, main): `if (transformedString == null) return;` keeps the original, any string - also an empty one - is written back and the stream truncated to it.** So a failed payload read used to serve a BLANK index.html to every client; realistic trigger: a File Transformation update that changes the payload shape (3.0.x is out, 2.5.9 installed). Fix: the callback returns `string?`, null on the read-failure and empty-contents paths. | code read | believe | read the FT plugin source; return `contents ?? string.Empty` is impossible (contents unknown) - safest is to rethrow so FT falls back to the original file, if it does | the whole index.html injection if FT does not fall back |
...
blocker 0 · bug 0 (S1-02 downgraded to smell/pattern in Session 138) · risk 2 (S1-01, S1-03 - each
...
| ID | file:line | class | sev | finding | evidence | know/believe | recommendation | a fix could break |
|---|---|---|---|---|---|---|---|---|
| S2-01 | `RenderArt-v1.js:720-731` with `FileTransformationRegistrar.cs:373-374` | prehiding not released | bug - FIXED (Session 138, scripts deploy #75) | While LogoArt intervenes for at least one type, index.html carries `.detailLogo:not(.artworkplus-logoart-vanilla){visibility:hidden}`. `start()` adds the release class only on `ZeroIntervention`; the paths `fetch` error (server restarting, 500) and `!result.IsApplicable` (item types without a LogoArt block: Trailer, Playlist, Photo, LiveTV Program/Channel, Audio) `return` WITHOUT releasing - the vanilla logo of such a page stays invisible until the next navigation. Not reproducible in the user's library today: no Trailer/Playlist with a logo, the Program page renders no logo, and the current config injects no prehiding (`prehideStyle=false` measured). **Fix:** `start()` releases the vanilla slot on a failed fetch and on `IsApplicable=false` (after `findLogo`); two new render tests red before / green after. | code read (two return paths before the release); live check on a Program page: no logo rendered, no prehiding active | believe | release the class on every early return after `findLogo` (error, not applicable), same as `ZeroIntervention` | nothing - `Hide` keeps its own path |
| S2-02 | `RenderArt-v1.js:281, 443, 802` + `Core.watchForNavigation` / heartbeat | duplicate work | smell - FIXED (Session 138, scripts deploy) | Characterart, RedCarpet and LogoArt each fetch their endpoint twice on every detail-page load (measured 1 781 ms and 1 958 ms after reload, 7-12 ms each): `viewshow` schedules once, the initial poll / heartbeat schedules again; the run token makes the first result dead. Harmless, 2x server resolve per page. **Fix:** `Core.isBootRaceRepeat` (3 s window, same hash) - the three RenderArt modules ignore the viewshow that follows their cold-start timer on a hard reload; hash changes, poll and heartbeat unchanged. Render test: 6 fetches before, 3 after; a later navigation still runs. | request timeline | know | debounce `schedule()` by hash (skip when the hash and item are unchanged and a run is in flight) | the recovery path of the heartbeat if the guard is too strict - keep the recover call unconditional |
...
| S2-04 | `Backdrops-v1.js:235, 565, 677, 795, 889, 999, 1129` | boot burst | smell - FIXED (Session 138, scripts deploy) | Seven owner IIFEs each schedule `Core.dispatchNavigationNow` at 1 200 ms; each dispatch calls all 7 listeners (`source==='viewshow'` bypasses the hash dedupe) = 49 listener runs at boot. The owners dedupe by visit id - `settings` was fetched once (measured). Cost negligible; duplicated boiler-plate of the copy-per-owner pattern. **Fix:** one boot dispatch armed in `Core.onNavigation` at the first registration; the seven per-owner timers removed. Render test: the probe listener ran 7x before, 1x after. | code + measured single settings fetch | know | one boot dispatch in Core after the last owner registers | nothing (idempotent today) |
| S2-05 | `Core-v1.js:871, 1608`, `Posters-v1.js:4617`, `RenderArt-v1.js:444`, heartbeats `RenderArt-v1.js:283, 804`, tile scanner `Posters-v1.js:1181` | standing observers | smell / design - scanner FIXED (Session 138, scripts deploy), polls = design | Permanently: three 1 s hash polls (Core dispatcher, Posters `watchForNavigation`, RenderArt `watchForNavigation`), two 750 ms heartbeats with two body-wide debounced MutationObservers, one body-wide UNdebounced tile scanner (`querySelectorAll` of three card selectors per mutation batch). Measured hidden on the 580-card library: 8 scans, 6.8 ms total (0.85 ms each), 12 mutation records; visible-tab lazy loading adds mutations (blurhash canvases) - not measured. Historic: `watchForNavigation` predates `Core.onNavigation` (Session 120), both kept. **Visible-tab measurement (516-card library, lazy loading + 6 scroll steps, 12.8 s window):** before 513 document-wide scans / 230 ms, long tasks 1 494 ms; after (the observer searches only the ADDED subtrees, `scanAdded`; the viewshow / boot scans still walk the document) 10 document scans / 5.7 ms, long tasks 1 210 ms, 131 tiles pending/layered as before. Tile suite 29/29, CaseMod 124/124. The three 1 s hash polls and two heartbeats cost nothing measurable - left as they are (design, legacy safety nets). | code + measurement | know (hidden), believe (visible) | route Posters/RenderArt through `Core.onNavigation` (one poll), debounce the scanner by a frame | navigation timing of the detail modules (the 1 s poll is their backup); the scanner's pre-paint pending mark must stay synchronous for the first insertion |
| S2-06 | `Posters-v1.js:3926-3940` | delayed listener release | smell - FIXED (Session 138, scripts deploy) | The CaseMod `resize` listener and `ResizeObserver` of a page visit remove themselves only when the NEXT resize / observe callback sees a stale generation; until a resize happens the closures keep the old page's card subtree alive. Heap over the 27-minute stress run: 35 -> 38 MB, peak 56, no trend - negligible in practice. **Fix:** `onGenerationEnd()` disposers in Posters, run by `scheduleDetail()` before the generation bumps; CaseMod registers the resize listener + ResizeObserver release there (the lazy self-removal stays as a second net). CaseMod test 124: 1 add / 0 removes before, removed at generation end after. | code; heap trend | believe | remove both in the generation-change path instead of lazily | nothing |
| S2-07 | `Posters-v1.js:2500-2513, 2677-2718`, `Core-v1.js:1895` | dead code | smell - FIXED (Session 138, scripts deploy) | `mat4ColMulVec`, `rowVecMulMat4`, `computeKodiMatrix3dStringWithOffset` (~45 lines, CaseMod matrix experiments) are never called; Core exports `ensureBodyIsPositioned` which nothing uses. **Fix:** the three dead CaseMod matrix helpers removed (~45 lines); the Core export `ensureBodyIsPositioned` is KEPT - a public Core API could be used by external scripts, removing it is not a no-risk change. CaseMod suite 123/123, node check, run_checks 21/21. | reference count = definition only | know | delete (the CaseMod suite's 123 tests guard the live matrix path) | nothing |
...
| S2-09 | `Controllers/CoreScriptController.cs`, `PostersPlusController.cs`, `RenderArtController.cs`, `BackdropsController.cs` (`script.js`) | script caching | risk - FIXED (Session 138, deploy #79) | The four script endpoints send NO Cache-Control / ETag, so browsers cache them heuristically: after a deploy an open browser can run a NEW feature script against an OLD Core (or vice versa) until its heuristic cache expires. Observed live: after the S2-02/S2-04 scripts deploy the page still had the old Core -> `Core.isBootRaceRepeat is not a function` -> Characterart / RedCarpet / LogoArt did not run until a real reload; the window lasted from the scripts copy until the full deploy + reload. **Fix:** `Helpers/ScriptCaching` - `Cache-Control: no-cache` + ETag (file mtime/size; the assembly MVID for the embedded Core) with 304 on the four script.js endpoints; guard `tests/diagnostic_script_endpoints_cache.py` (22/22). Proof after: second request with If-None-Match -> 304, headers present. | live: `typeof ArtworkPlusCore.isBootRaceRepeat === 'undefined'` while the server already served it; `cache-control=null` | know | `Cache-Control: no-cache` + ETag (file mtime/size) on all four script endpoints, so every page load revalidates (304 when unchanged); and the rule: a Core change is a FULL deploy (Core is embedded), never `deploy.py scripts` alone | nothing (one extra conditional request per page load) |
...
| ID | file:line | class | sev | finding | evidence | know/believe | recommendation | a fix could break |
|---|---|---|---|---|---|---|---|---|
| S3-01 | `configPage.html:8312-8314` (`parseFloat(...) \|\| 0`), inputs `min="0"`; `RenderArt-v1.js:256, 641`, `Posters-v1.js:1703`; no server floor | numeric floor | bug - FIXED (Session 138, deploy #75) | A cleared or 0 "Display duration" saves as 0 (empty -> `parseFloat` -> 0, and `min="0"` accepts 0). Characterart, LogoArt (Characterart stage) and the Extraposter detail slideshow then run `setTimeout(showNext, 0)` - an image switch every timer tick (~4 ms) on that page, CPU-bound, until navigation. Backdrops (`Core-v1.js:1815`, floor 1.5 x fade) and the Extra tiles (`Posters-v1.js:2074`, floor 250 ms) have floors; the server clamps only `OpenAngleDegrees`. Never observed - the user never saved 0. **Fix:** `Helpers/Timing.FloorCycle` (250 ms, the tiles' floor) applied where Characterart, LogoArt and the Extraposter detail + library DTOs are built; page and engines untouched. Proof: temp config with cycle 0 (restored byte-identical) -> DTO cycle 0 before, 250 after. | code read at three sites | believe (a probe would need a saved 0) | a floor on the server where the DTO is built (`Math.Max(..., 250)`) or `min="100"`-style validation in `epGetFieldValue`; keep both engines untouched | nothing if the floor sits in the server DTO; a page-side floor changes what a saved 0 means |
| S3-02 | `configPage.html:8289-8316`, save path 11589-11611 | input validation | smell - FIXED (Session 138, deploy #80) | Numeric fields are read with `parseFloat` and no range check - the HTML `min` / `max` / `step` attributes only guard the spinner, typed values (negative, huge, decimals where an int is expected) reach the server and are cast by the JSON binder (a decimal into an `int` field is rejected by the server as a 400 without a page message). **Fix:** `epGetFieldValue` clamps `type=number` values to the element's min / max and rounds where the step is a whole number (signed offsets without min stay as typed, data-numeric text fields untouched); the three People ms inputs got min/step like their Backdrops twins. Test S138c red (−500 / 999 / 2500.7 passed through) -> green. | code read | know | one `clamp(min, max)` in `epGetFieldValue` from the element's attributes | any field whose attributes are stricter than the server actually needs - check the 211 `min` attributes first |
| S3-03 | `configPage.html:11356-11369` (export), `11406-11463` (import) | transfer gap | bug - FIXED (Session 138, deploy #74) | The Export / Import code carries EP_FIELDS + the six format csv but NOT the 28 per-case-type tune values (`*CaseTune*`, `CaseMod3DTune*`) - the 3D case geometry a user tuned is silently left at the target server's defaults after an import. The status text "Saved N + 4 settings" also counts 4 where 6 csv are written. **Fix:** the code carries the 28 case-tune values (active type from the fields, the others from the last loaded state), import restores them (active -> fields, others -> a buffer written on the next save); status counts EP_FIELDS + 6 + 28. Test S138b red before (1 of 28), green after. | code read (export builds from EP_FIELDS + 6 csv only) | know | include the 28 tune properties from `epLastLoadedConfig` + the active fields in the export, apply them in the import | nothing (additive) |
| S3-04 | `configPage.html:11358` | secret in export | risk - FIXED (Session 138, deploy #74) | `PeopleBackdropsApiKey` is an EP_FIELDS text field and therefore part of the Export code (base64 of JSON, not encrypted) - a code pasted into a forum or repo ships the Wallpapers.com key. Never considered, not a decision. **Fix:** `PeopleBackdropsApiKey` is deleted from the export and ignored on import; the status line says "API key not included". Test S138b. | code read | know | exclude the key from the export (and say so in the status line), or mask it | anyone relying on the export to move the key |
| S3-05 | `configPage.html:10957-10964` | poll teardown | smell - FIXED (Session 138, deploy #73) | The create-logos status poll (1 req/s) starts when a job runs and stops only when the job ends or the request fails; leaving the plugin page keeps it running until then (the config page has no `viewhide` teardown). Cost: one small admin GET per second during a job. **Fix:** the poll clears on `viewhide` of the page element and a running job is picked up again on `viewshow` (Jellyfin viewManager events verified in source). Test S138: red before (2 polls after viewhide), green after (0, one pickup). | code read | know | clear the interval on `viewhide` / `pagehide` and re-arm on `pageshow` | nothing |
...
| ID | file:line | class | sev | finding | evidence | know/believe | recommendation | a fix could break |
|---|---|---|---|---|---|---|---|---|
| S4-01 | `ArtworkPlus.csproj` (SixLabors.ImageSharp 3.1.7) | dependency advisory | risk - FIXED (Session 138, deploy #76) | NU1902: 3.1.7 carries a known moderate advisory (GHSA-rxmq-m78w-7wmc). It matters here more than in most plugins: ImageSharp decodes images DOWNLOADED from the web (People Backdrops aspect check + text detector, `PeopleBackdropsController.cs:1379-1394`), i.e. untrusted input, reachable through the anonymous endpoint (S1-01). The 3.1.x line stays under the free split licence without a key (the pin at 3.1.7 was for the 4.0 licence-key rule, not for 3.1.7 itself). **Fix:** SixLabors.ImageSharp 3.1.7 -> 3.1.12 (latest 3.1.x, no licence key); NU1902 gone from the build; **Correction (same session):** deploy #76 did NOT ship the new DLL - `tools/deploy.py` never had `SixLabors.ImageSharp.dll` in its copy set (nor the CLAUDE.md compile snippet), the server kept 3.1.7 and the first "proof" ran on the old library. Fixed in `238b040` (copy set + snippet), deploy #77, deployed file version 3.1.12 verified, population flow exercised on 3.1.12 (Anne Hathaway, no cache: 10 accepted, decode + text detection, 0 errors). Lesson: a dependency bump is only deployed when the deploy tool ships the dependency - the tool's copy set belongs in every dependency dossier. | build warning; csproj comment; download path | know | bump to the latest 3.1.x patch (no licence key, same API), rebuild, run the People smoke | nothing expected - patch line; verify `PassesAspectRatioCheck` and the text detector on the live People fixture |
...
| S4-03 | `ArtworkPlus.csproj` SkiaSharp reference; `bin/Release/net8.0/runtimes` | packaging trap | smell - FIXED (Session 138) | `ExcludeAssets="runtime"` on SkiaSharp keeps SkiaSharp.dll out of the output, but the transitive `SkiaSharp.NativeAssets.Win32/macOS` still drop 39 MB of native `libSkiaSharp.*` into `runtimes/`, and `deps.json` lists them. deploy.py does not copy `runtimes/`, so the live server is clean; anyone zipping `bin/Release/net8.0` for a release would ship 39 MB of natives next to a host that already has them - the "native DLL loaded as managed -> Malfunctioned" class the csproj comment describes for ONNX. **Fix:** `ExcludeAssets="runtime;native"` on SkiaSharp - no `runtimes/` folder, no libSkiaSharp in deps.json; guard `diagnostic_csproj_native_assets.py`. | build output listing; deps.json grep (4 hits) | know | `PrivateAssets="all"` / `ExcludeAssets="runtime;native"` on SkiaSharp, or exclude `runtimes/**` from the release zip step | only the LogoArt creator if the host's SkiaSharp were ever missing - it is not (host folder lists SkiaSharp.dll + libSkiaSharp.dll) |
| S4-04 | `meta.json` | catalog metadata | doc - FIXED (Session 138) | `version` 1.0.0.0 since the first deploy (69 deploys), `changelog` "Initial release", `timestamp` 0001-01-01, `owner` empty, `description`/`overview` "Five independent features" (no CaseMod, Library View Backdrops, Persons, LogoArt...). Fine for a self-deployed folder, wrong for the public repo / a release. No release process exists yet (design: never scoped). **Fix:** meta.json description / overview / changelog / timestamp / owner updated (version stays 1.0.0.0 - a version bump renames the plugin folder and is a release decision). | file read | know | a release checklist: version bump + changelog + timestamp + overview at the milestone tag | nothing at runtime; a version change renames the plugin folder (`ArtworkPlus_x.y.z.w`), deploy.py's target path is hard-coded to 1.0.0.0 |
...
| ID | file:line | class | sev | finding | evidence | know/believe | recommendation | a fix could break |
|---|---|---|---|---|---|---|---|---|
| S5-01 | `docs/artworkplus-feature-map.md` (delivery table, config-prefix column, cross-cutting) | drift | doc - FIXED (Session 138) | Counts drifted since the map was generated in Session 114: "PluginConfiguration.cs (369 settings)" vs 763 today; prefix counts off for 10 of 19 prefixes (Extraposter "7 + 4x7 + 2 sync" vs 101 incl. the 48 Also-on fields, Extrakeyart 87, Backdrops 130, PeopleBackdrops 19 vs 24, BackdropsFavorites 39 vs 60, BackdropsFavoritesPeople 3 vs 10, CaseMod 27 vs 28, Characterart 95 vs 97, Genre/Studio/Tag +1 each = Random start, BackdropsLibrary 16 vs 15); "Fibel rules 0-26" (27 exists, 27b/28 too); "Server side: `config\logging.json` overrides ... to Debug" - it has been Information since Session 127e (lesson B25, verified in the live file). The Import-button `.raised` gap is still true. **Fix:** all prefix counts and the total regenerated from the code, the logging line corrected (Information + the Debug lever); guard `diagnostic_feature_map_counts.py` in run_checks so the map cannot drift silently. | counts recomputed from C#; live logging.json read | know | regenerate the counts by script (the map says "generated from the code" - make that a tool, `tools/feature_map_counts.py`, and run it in run_checks as informational) | nothing |
| S5-02 | `CLAUDE.md:20, 103, 142-143` | drift | doc - FIXED (Session 138) | "rules (0-26)", "`test_configpage.py` 406 tests", "run_checks.py --all 19/19 (406 config tests, 27 tile scenarios" - today: 413 tests, 20/20, 29 scenarios, rules 0-28. The state list is otherwise current (Sessions 135/136 appended). **Fix:** CLAUDE.md test numbers and rule range updated (417 / 24 checks / 29 / 124 / 0-28). | suite output of this session | know | update the three lines with the next state edit | nothing |
| S5-03 | `README.md` | empty | doc - FIXED (Session 138) | The public repository's README is empty (0 lines) - the repo has no user-facing description, install note or feature list; `meta.json` (S4-04) is the only description and it is stale. **Fix:** README.md written (features, install incl. File Transformation + data-folder scripts, build, docs, licence). | file size | know | a README from the feature map's feature list + install (data-folder scripts, File Transformation dependency) | nothing |
...
|---|---|---|
| blocker | 0 | - |
| bug | 3 (S1-02 downgraded in its /fix dossier) | S2-01 (LogoArt prehiding not released, narrow, believe), S3-01 (Display duration 0 spins the rotation, believe), S3-03 (Export/Import loses the 28 case-tune values) |
