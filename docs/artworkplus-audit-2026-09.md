...
| S1-13 | `FileTransformation/FileTransformationRegistrar.cs:494-496` | error path | risk (believe) | Found while fixing S1-08: if reading `payload.contents` throws, the transform returns `string.Empty` - depending on how the File Transformation plugin treats an empty return, the SPA index could be served blank. Never happened (the read never failed: 0 such log lines); needs a look at the File Transformation plugin's handling of an empty callback result before deciding whether to return the original or throw. | code read | believe | read the FT plugin source; return `contents ?? string.Empty` is impossible (contents unknown) - safest is to rethrow so FT falls back to the original file, if it does | the whole index.html injection if FT does not fall back |
