...
| S1-04b | `CaseModController.cs:465`, `LogoArtController.cs:699, 831` (`GetFont`, `ServeFile`) | deferred open | smell | Found by the S1-04 fix: `PhysicalFile(...)` opens the file when the RESULT executes, outside the action's try/catch - a locked font answers 500 (probe: `/LogoArt/font/Signature/Activity.otf` locked -> 500, unlocked -> 200) although the action is wrapped. Three sites left (CaseMod texture, LogoArt font, LogoArt ServeFile for characterart / person images). | probe + code | know | open the stream in the action (`File(System.IO.File.OpenRead(path), type)`) as done for `ServeLocalImage` / `child-image` | nothing (same headers, same bytes) |
