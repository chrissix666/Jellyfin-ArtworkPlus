...
| S2-09 | `Controllers/CoreScriptController.cs`, `PostersPlusController.cs`, `RenderArtController.cs`, `BackdropsController.cs` (`script.js`) | script caching | risk (found in Session 138k) | The four script endpoints send NO Cache-Control / ETag, so browsers cache them heuristically: after a deploy an open browser can run a NEW feature script against an OLD Core (or vice versa) until its heuristic cache expires. Observed live: after the S2-02/S2-04 scripts deploy the page still had the old Core -> `Core.isBootRaceRepeat is not a function` -> Characterart / RedCarpet / LogoArt did not run until a real reload; the window lasted from the scripts copy until the full deploy + reload. | live: `typeof ArtworkPlusCore.isBootRaceRepeat === 'undefined'` while the server already served it; `cache-control=null` | know | `Cache-Control: no-cache` + ETag (file mtime/size) on all four script endpoints, so every page load revalidates (304 when unchanged); and the rule: a Core change is a FULL deploy (Core is embedded), never `deploy.py scripts` alone | nothing (one extra conditional request per page load) |
