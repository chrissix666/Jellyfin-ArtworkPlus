...
| Extraposter | detail page + library grid tiles, overlay slideshow; Session 123: settings per Movies/TV shows × Detail page/Library views (three collapse levels), Library "Synchronize" = one page clock (first appearance and every change on a tick) or independent tiles | ExtraModule | `Extraposter`: `{itemId}/quickcheck`, `{itemId}?type=`, `batch`, `{itemId}/image/{fileName}` | `Extraposter*` (7 + 4×7 + 2 sync) | extraposter | ❓ built + replica-tested (test_library_tiles S13/S14), live look pending | |
